...
{
	let mut output = Vec::with_capacity(source.len() + source.len() / 10);
	let new_finger_option =
		substitute_impl_one_step(0, &mut output, source.as_bytes(), &(0..source.len()), variables, &|x| {
			x.as_ref().as_bytes()
		})?;
	...
	// SAFETY: Both source and all variable values are valid UTF-8, so substitation result is also valid UTF-8.
	let output_str = unsafe { String::from_utf8_unchecked(output) };
...
mod test {
	...
	fn test_invalid_escape_sequence() {
		...
		assert!(e.source_highlighting(source) == concat!(r"  Hello \world!", "\n", r"        ^^", "\n",));

		let source = r"Hello \❤❤";
		let_assert!(Err(e) = substitute(source, &map));
		assert!(e.to_string() == r"Invalid escape sequence: \❤");
		assert!(e.source_highlighting(source) == concat!(r"  Hello \❤❤", "\n", r"        ^^", "\n",));
	...
	fn test_missing_variable_name() {
		...
		let_assert!(Err(e) = substitute(source, &map));
		assert!(e.to_string() == r"Missing variable name");
		assert!(e.source_highlighting(source) == concat!(r"  Hello ${:fallback}!", "\n", r"        ^^", "\n",));
	...
	fn test_unexpected_character() {
		...
		assert!(e.source_highlighting(source) == concat!("  Hello ${name)!\n", "              ^\n",));
	...
	fn test_missing_closing_brace() {
		...
		let_assert!(Err(e) = substitute(source, &map));
		assert!(e.to_string() == "Missing closing brace");
		assert!(e.source_highlighting(source) == concat!("  Hello ${name:fallback\n", "         ^\n",));
	}
	...
	fn test_unicode_invalid_escape_sequence() {
		...
		assert!(e.source_highlighting(source) == concat!(r"  emoticon: \（ ^▽^ ）/", "\n", r"            ^^^", "\n",));
	}
}
